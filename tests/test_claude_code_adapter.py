...
class TestClaudeCodeAdapterParsing:
    ...
    def test_token_usage(self, claude_code_session):
        ...
        # token_usage_in is now ONLY non-cached input tokens
        # cache_read_tokens and cache_create_tokens tracked separately
        assistant_turns = [t for t in session.turns if t.role == "assistant"]
        first_assistant = assistant_turns[0]
        assert first_assistant.token_usage_in == 1500  # non-cached input only
        assert first_assistant.cache_read_tokens == 500  # cache_read separate
