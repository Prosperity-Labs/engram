...
class ClaudeCodeAdapter(AgentAdapter):
    ...
    def parse_event(self, event: dict) -> Turn | None:
        ...
        if entry_type == "assistant":
            return self._parse_assistant(message, ts, tokens_in, tokens_out)

        if entry_type == "summary":
            return self._parse_summary(event, message, ts, tokens_in, tokens_out)
    ...
    def _parse_assistant(
        self, message: dict, ts: str | None, tokens_in: int, tokens_out: int
    ) -> Turn | None:
        content_blocks = message.get("content", [])
